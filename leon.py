...
class Around:
    def __init__(self, robot, game):
        self.robot = robot
        self.game = game
        self.locs = rg.locs_around(robot.location,
                                   filter_out=('invalid', 'obstacle'))
        self.robots = {loc: game.robots[loc] for loc in self.locs if
                       loc in game.robots}
        self.free_locs = [l for l in self.locs if l not in self.robots]
        self.opponents = {loc: r for loc, r in self.robots.items() if
                          r.player_id != robot.player_id}
        self.friends = {loc: r for loc, r in self.robots.items() if
                        r.player_id == robot.player_id}
...
class Robot:
    ...
    def next_loc(self):
        ...
        possible_locs = [loc for loc in self.around.free_locs if
                         loc not in self.last_fields]
